...
securityCommand = "aws ec2 create-security-group --group-name script-{}-security-group --description 'Security group for administering build pipeline resources --vpc-id {} > script-{}-security-group-id.json".format(configData['iam-user'],configData['vpc-id'],configData['iam-user'])
#DEBUG print statement
# print(securityCommand)
# execute command
os.system(securityCommand)

f2 = open("script-{}-security-group-id.json".format(configData['iam-user'])
